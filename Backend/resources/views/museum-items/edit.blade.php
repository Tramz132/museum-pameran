...
@section('title', 'Edit Barang Museum')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            <h2 class="text-xl font-bold text-slate-800">Edit Koleksi Barang</h2>
            <p class="text-sm text-slate-500">Perbarui informasi barang koleksi museum Anda di bawah ini.</p>
        </div>
    </div>

    <!-- Form Card -->
    <x-card class="bg-white">
        <form method="POST" action="{{ route('admin.museum-items.update', $museumItem) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang', $museumItem->nama_barang) }}"
                ...
                <div>
                    ...
                    <input type="text" name="kode_barang" id="kode_barang" value="{{ old('kode_barang', $museumItem->kode_barang) }}"
                ...
                <div>
                    ...
                    <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $museumItem->kategori) }}"
                ...
                <div>
                    ...
                    <input type="text" name="asal" id="asal" value="{{ old('asal', $museumItem->asal) }}"
                ...
                <div>
                    ...
                    <input type="text" name="tahun" id="tahun" value="{{ old('tahun', $museumItem->tahun) }}"
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="Tersedia" {{ old('status', $museumItem->status) === 'Tersedia' ? 'selected' : '' }}>Tersedia</option>
                        <option value="Dipinjam" {{ old('status', $museumItem->status) === 'Dipinjam' ? 'selected' : '' }}>Dipinjam</option>
                        <option value="Perbaikan" {{ old('status', $museumItem->status) === 'Perbaikan' ? 'selected' : '' }}>Perbaikan</option>
                    </select>
                    @error('status')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                          class="w-full px-4 py-2.5 text-sm bg-slate-50 border @error('deskripsi') border-rose-500 focus:ring-rose-500 @else border-slate-200 focus:ring-blue-500 @enderror rounded-xl focus:outline-none focus:ring-2 focus:border-transparent transition-all">{{ old('deskripsi', $museumItem->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Upload Foto -->
            <div class="space-y-4">
                <label class="block text-sm font-semibold text-slate-700">Foto Barang</label>
                
                @if ($museumItem->foto)
                    <div class="flex items-center space-x-4 p-4 bg-slate-50 border border-slate-150 rounded-2xl w-fit">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-slate-500">Foto saat ini</p>
                            <p class="text-xs text-slate-400 mt-0.5">Unggah foto baru di bawah jika ingin mengganti.</p>
                        </div>
                    </div>
                @endif

                <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors">
