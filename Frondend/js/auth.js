...
// Deteksi path dasar root secara dinamis agar kompatibel dengan protokol file:/// (double click) dan http:// (Live Server)
function getRootPath() {
    if (window.location.protocol === 'file:') {
        const path = window.location.pathname;
        const index = path.indexOf('/Frondend/');
        if (index !== -1) {
            return path.substring(0, index) + '/Frondend';
        }
    }
    return '/Frondend';
}
...
function requireAuth(allowedRoles = []) {
    ...
    if (allowedRoles.length > 0 && !allowedRoles.includes(user.role)) {
        ...
        if (user.role === 'admin') window.location.href = `${ROOT_PATH}/admin/dashboard.html`;
        else if (user.role === 'staff') window.location.href = `${ROOT_PATH}/staff/dashboard.html`;
        else if (user.role === 'panitia') window.location.href = `${ROOT_PATH}/panitia/dashboard.html`;
        else logout();
    }
}
...
function injectLayout() {
    ...
    if (sidebarEl) {
        ...
        if (user.role === 'admin') {
            menuItems = `
                <a href="${ROOT_PATH}/admin/dashboard.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-dashboard">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4zM14 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2v-4z"/></svg>
                    <span class="font-medium">Dashboard</span>
                </a>
                <a href="${ROOT_PATH}/admin/items.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-items">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
                    <span class="font-medium">Kelola Barang</span>
                </a>
                <a href="${ROOT_PATH}/admin/users.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-users">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    <span class="font-medium">Kelola User</span>
                </a>
                <a href="${ROOT_PATH}/admin/loans.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-loans">
            ...
            menuItems = `
                <a href="${ROOT_PATH}/staff/dashboard.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-dashboard">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4zM14 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2v-4z"/></svg>
                    <span class="font-medium">Dashboard</span>
                </a>
                <a href="${ROOT_PATH}/staff/verifications.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-verifications">
            ...
            menuItems = `
                <a href="${ROOT_PATH}/panitia/dashboard.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-dashboard">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4zM14 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2v-4z"/></svg>
                    <span class="font-medium">Dashboard</span>
                </a>
                <a href="${ROOT_PATH}/panitia/catalog.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-catalog">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z"/></svg>
                    <span class="font-medium">Katalog Barang</span>
                </a>
                <a href="${ROOT_PATH}/panitia/loans.html" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 nav-link" id="nav-loans">
